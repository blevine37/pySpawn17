...
def prop_not_first_step(self, zbackprop):
    """Velocity-verlet integrator with Langevin parameters (AMehmood)"""
    ...
    # Langevin thermostat using method Schneider and Stoll, Phys Rev B, 17, 1302 (1978).
    # Same implementation as in lammps
    # F = Fc + Ff + Fr
    # Fc = Conservative force from electronic structure
    # Ff = -(m/damp)*v
    # Fr = sqrt(2*Kb*T*m/(dt*damp))
    # NOTE: Random numbers are used to randomize the direction and magnitude of this force
    
    Kb = 3.166811563E-6 # Boltzmann constant in hartree/K
    T = self.get_sim_temp()
    damp = self.get_lan_damp()
    
    Ff = -(m/damp)*v_tphdt
    Fr = np.sqrt(2.0*Kb*T*m/(dt*damp))*np.random.randn(*f_tpdt.shape)
    f_tpdt = f_tpdt + Ff + Fr    
